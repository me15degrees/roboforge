...
class Sumo:
    ...
    def rununtilstalled(self, speed):
        self.r_motor.run_until_stalled(speed, then=Stop.COAST, duty_limit=None)
        self.l_motor.run_until_stalled(speed, then=Stop.COAST, duty_limit=None)
        self.r1_motor.run_until_stalled(-speed, then=Stop.COAST, duty_limit=None)
        self.l1_motor.run_until_stalled(-speed, then=Stop.COAST, duty_limit=None)
    ...
    while not flag:  # Espera pelo botão central.
        for button in ev3.buttons.pressed():
            ...
            if button == Button.CENTER:
                flag = 1
    while True:
        robo_sumo.rununitlstalled(200,40)
