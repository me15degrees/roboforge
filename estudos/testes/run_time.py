...
class Sumo:
    ...
    def runtime(self, speed, time):
        self.r_motor.run_time(speed, time, then=Stop.HOLD, wait=True)
        self.l_motor.run_time(speed, time, then=Stop.HOLD, wait=True)
        self.r1_motor.run_time(-speed, time, then=Stop.HOLD, wait=True) 
        self.l1_motor.run_time(-speed, time, then=Stop.HOLD, wait=True) 
